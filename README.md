...
Сервис обеспечивает эффективное обновление и очистку данных, используя структуры `Map` и `Deque`.
Альтернативы, такие как Sparse Table и матрицы минимумов и максимумов, были отклонены из-за их недостатков в производительности и сложности реализации.
...
2. **getMin(interval: string): number | null**
    - Возвращает минимальное значение за заданный интервал времени. Если `Deque` для этого интервала пуст, возвращается `null` (при необходимости можно возвращать `infinity`).

3. **getMax(interval: string): number | null**
    - Возвращает максимальное значение за заданный интервал времени. Если `Deque` для этого интервала пуст, возвращается `null` (при необходимости можно возвращать `infinity`).
...
3. **Очистка устаревших данных**
    - Время выполнения: `O(n)`, где `n` — количество элементов, так как каждый элемент проверяется и потенциально удаляется только один раз. Не кретично с утом заданного интевала и кол-ва записей за интервал.
